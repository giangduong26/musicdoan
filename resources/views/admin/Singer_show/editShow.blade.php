		...
		<div class="form-three widget-shadow">
			<h4>Edit Show</h4><br>
				<div class=" panel-body-inputin">	
					...
					@endif
					<form class="form-horizontal" method="POST" action="admin/Singer_show/editShow/{{$show->id_show}}" enctype="multipart/form-data">
						{{csrf_field()}}
							...
							<div class="col-md-8">
								<div class="input-group">
									...
									<select class="form-control1" name="song">
										...
										@foreach($song as $s)
										<option 
										@if($show->song->id_song == $s->id_song) 
											{{"selected"}}
										@endif
										value="{{$s->id_song}}">{{$s->name_song}}
										</option>
										@endforeach
									</select>
								</div>
							...
							<div class="col-md-8">
								<div class="input-group">							
									...
									<select class="form-control1" name="singer">										
										...
										<option 
										@if($show->singer->id_singer == $sg->id_singer) 
											{{"selected"}}
										@endif
										value="{{$sg->id_singer}}">{{$sg->nickname}}
										</option>
										@endforeach
									</select>
								</div>
